...
class EditWarmupExercise extends StatefulWidget {

  static const routeName = '/edit_warmup_exercise';

  const EditWarmupExercise({ Key? key }) : super(key: key);

  @override
  State<EditWarmupExercise> createState() => _EditWarmupExerciseState();

}

class _EditWarmupExerciseState extends State<EditWarmupExercise> {
  ...
  // Delete Warmup Exercise from Database
  Future _deleteExercise() async{

    try{
      FocusScope.of(context).unfocus();

      final docExerciseData = FirebaseFirestore.instance.collection('warmupExercises').doc(_selectedExerciseId);
      await docExerciseData.delete();

      
      Utils.showToast(' Warmup Exercise Deleted Successfully');
  ...
  // Update Warmup Exercise in Database
  Future _updateExercise() async{
    ...
    try{
      ...
      Utils.showToast('Warmup Exercise updated successfully');
  ...
  // Form for Editing Warmup Exercise
  Widget _editWarmupExerciseForm(){
     return ListView(
    ...
    return showDialog<void>(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Text("Are you sure you want to delete this Warmup Exercise?"),
          actions: [
            TextButton(
              child: Text("Yes"),
              onPressed:  () {
                _deleteExercise();
                Navigator.popUntil(context, ModalRoute.withName('/admin_warmup_exercises'));
              },
            ),
            TextButton(
              child: Text("No"),
  ...
  //Get Data of Current Warmup Exercise
  void _getdata() async {

    final data = FirebaseFirestore.instance.collection('warmupExercises').doc(_selectedExerciseId);
    final snapshot = await data.get();

    WarmupExercises warmupExerciseData = WarmupExercises.fromJson(snapshot.data()!);
  ...
  void initState() {
    ...
    WidgetsBinding.instance!.addPostFrameCallback((_) {
      ...
      _selectedExerciseId = screenData['warmupExerciseId']!;
